...
console.log(githubData_joe);

// ---------- #1 How many total commits were made in all of Steve's PUSH events?

//loop through outer array. Find commits array in each object
//and find its length

let commits_joe = 0;

githubData_joe.forEach( function(event_joe) {
  //event_joe represents each object in the array githubData_joe
  
  if (event_joe.payload.commits) {
    commits_joe += event_joe.payload.commits.length;
  }
});

console.log(`Answer 1... ${commits_joe} commits`);

// ---------- #2 How many of each event type are there?

let allEvents_joe = [];

//push all event types to an array
githubData_joe.forEach( function(event_joe) {
  allEvents_joe.push(event_joe.type);
});

//loop array to count each unique type
let eventCount = {}
//i represents the value at array index i (a type pushed from above)
allEvents_joe.forEach( function(i) {
  //key name takes itself plus one or is initially zero and adds one
  // to start the new key value pair
  eventCount[i] = (eventCount[i] || 0) + 1;
});

console.log('Answer 2...', eventCount);